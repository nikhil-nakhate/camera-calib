function [x X ind_orig ind_x ind_y] = extractGrid(I,frame,wintx,winty,fc,cc,kc,n_sq_x,n_sq_y,dX,dY)
...
for count = 1:4
   ...
   x = [x;xi]; y_new = [y_new;yi];
   plot(x,y_new,'-','color',[ 1.000 0.314 0.510 ],'linewidth',2);
   drawnow;
end
plot([x;x(1)],[y_new;y_new(1)],'-','color',[ 1.000 0.314 0.510 ],'linewidth',2);
drawnow; hold off;

% Refine selected corners to sub-pixel precision.
Xc = cornerfinder([x';y_new'],I,winty,wintx);
x = Xc(1,:)'; y_new = Xc(2,:)';

% Sort the corners.
x_mean = mean(x); y_mean = mean(y_new);
x_v = x-x_mean; y_v = y_new-y_mean;
theta = atan2(-y_v,x_v);
[junk,ind] = sort(theta);
...
% xy_corners_undist = ...
%    comp_distortion_oulu([(x_new' - cc(1))/fc(1);(y_new'-cc(2))/fc(2)],kc);
...
r2 = sum(XXu.^2);       
XX_new = (ones(2,1)*(1 + kc(1) * r2 + kc(2) * (r2.^2))) .* XXu;
XX_new(1,:) = fc(1)*XX_new(1,:)+cc(1);
XX_new(2,:) = fc(2)*XX_new(2,:)+cc(2);
Np = (n_sq_x+1)*(n_sq_y+1);

% Refine reprojected corners.
grid_pts = cornerfinder(XX_new,I,winty,wintx);
grid_pts = grid_pts-1;
