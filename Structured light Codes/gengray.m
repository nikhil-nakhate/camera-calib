...
for j=1:2
    for i=1:size(P{1},3);
        if(j==1)
            file_name1=['column_pattern_',num2str(k,'%0.02d'),'.tiff'];
            imwrite(double(P{j}(:,:,i)),file_name1);
            k=k+1;
            file_name2=['column_pattern_',num2str(k,'%0.02d'),'.tiff'];
            imwrite(double(1-P{j}(:,:,i)),file_name2);
            k=k+1;
            disp([file_name1,' and ',file_name2,'..written']);
        else
            file_name1=['row_pattern_',num2str(k,'%0.02d'),'.tiff'];
            imwrite(double(P{j}(:,:,i)),file_name1);
            k=k+1;
            file_name2=['row_pattern_',num2str(k,'%0.02d'),'.tiff'];
            imwrite(double(1-P{j}(:,:,i)),file_name2);
            k=k+1;
            disp([file_name1,' and ',file_name2,'..written']);
        end
