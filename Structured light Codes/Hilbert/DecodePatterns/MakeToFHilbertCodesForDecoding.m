...
% Make ToF coding curves based on Hilbert curves. Return codes, and also
% the segment index for each code point. 
...
function [Codes, SegmentIndices]  = MakeToFHilbertCodesForDecoding(K, dim, ord, delta, NDepths)             
...
%                                    3    3   0   3   0   3   3   3   0   1   1   1];               % 0 means all zeros, 1 means all ones, 2 means first Hilbert coordiante, 3 means second Hilbert coordinate, negatives means the corresponding Hilbert coordinates in the reverse order
    ...
    X                                   = hilbert(ord);
    [XPoints, SubSegmentIndices]    	= NormalizeAndExpandHilbertCurvesForDecoding(X, delta, PointsPerSegment);
    [Codes, SegmentIndices]             = PermMatrixToCodesForDecoding(PermutationMatrix, XPoints, SubSegmentIndices, size(X,2)-1);
