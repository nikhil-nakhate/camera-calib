...
%%%%%% space filling curves of the increased dimension. 
...
dim                     = 2;            %%% dimension of the hilbert curve
order                   = 1;            %%% order of the hilbert curve

nr                      = 768;          %%% number of rows in the projector
nc                      = 1024;         %%% number of columns in the projector
...
if(dim==2)
    X   =   IncreaseHilbertDimensionality2to4D(X);
elseif(dim==3)
    X   =   IncreaseHilbertDimensionality3to5D(X);
end


%%%%%%%%%%%%%%%%%%%%%%%% Pruning out duplicate points %%%%%%%%%%%%%%%%%%%%%

XPruned     = X(:, 1);
Thresh      = norm(X(:, 1) - X(:, 2)) * 0.99;
for i=1:size(X, 2)
    E = [];
    for j=1:size(XPruned, 2)
        E(j) = norm(X(:, i) - XPruned(:, j));
    end
    if(min(E(:))>Thresh)
        XPruned = [XPruned X(:, i)];
    end
...
for i=1:size(XFinal, 1)
    ...
    imname          = [projdirname, '\Dim-', sprintf('%03d', dim+2), '-Order-', sprintf('%03d', order), '-Image-', sprintf('%03d', i), '.bmp'];
...
%%% Saving the points
P       = XFinal;
save([projdirname, '\Dim-', sprintf('%03d', dim+2), '-Order-', sprintf('%03d', order), '.mat'], 'P');
clear P
...
for i=1:size(XFinal, 1)
    ...
    imname          = [projdirname, '\Dim-', sprintf('%03d', dim+2), '-Order-', sprintf('%03d', order), '-Circled-BlurSize-', sprintf('%03d', blurSize), '-Image-', sprintf('%03d', i), '.bmp'];
    imwrite(im2uint8(tmpIm), imname);
...
%%% Saving the points
P   = XCircle;
save([projdirname, '\Dim-', sprintf('%03d', dim+2), '-Order-', sprintf('%03d', order), '-Circled-BlurSize-', sprintf('%03d', blurSize), '.mat'], 'P');
clear P
...
for i=1:size(XFinal, 1)
    tmpIm           = repmat(XFinal(i, :), [nr 1]);
    imname          = [projdirname, '\Dim-', sprintf('%03d', dim+2), '-Order-', sprintf('%03d', order), '-Blurred-BlurSize-', sprintf('%03d', blurSize), '-Image-', sprintf('%03d', i), '.bmp'];
    imwrite(im2uint8(tmpIm), imname);
    figure;plot(XFinal(i, :))
end
...
%%% Saving the points
P   = XFinal;
save([projdirname, '\Dim-', sprintf('%03d', dim+2), '-Order-', sprintf('%03d', order), '-Blurred-BlurSize-', sprintf('%03d', blurSize), '.mat'], 'P');
clear P
