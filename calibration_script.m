...
addpath('C:\Users\user\Documents\MATLAB\Geometric\lib\mlStructuredLight\mlStructuredLight\utilities');

% Select projector calibation images for inclusion.
useProjImages = 1:16; %[1:5,7:9];
...
proj_cal_nX = 5;         % number of rectangles along x-axis
proj_cal_nY = 7;         % number of rectangles along y-axis
proj_cal_dX = 33.0;     % length along x-axis (mm) (between checkerboard rectangles) [406.4]
proj_cal_dY = 33.0;     % length along y-axis (mm) (between checkerboard rectangles) [330.2]

basedirname     = ['C:\Users\user\Documents\MATLAB\Geometric'];
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% load([basedirname, '\cam\calib_data.mat']);
% 
% % Display the first camera calibration image.
% I1 = imread([basedirname, '\cam\IMG_4927.tif']);
% figure(1); image(I1); axis image;
% hold on;
...
% nx{camIndex}       = nx;
% ny{camIndex}       = ny;
% n_sq_x_1{camIndex} = n_sq_x_1;
% n_sq_y_1{camIndex} = n_sq_y_1;
% dX{camIndex}       = dX;
% dY{camIndex}       = dY;
...
% camIndex = 1; bounds = {}; projValue = {}; inv_color_calib = {};
...
%    end
%    inv_color_calib{camIndex} = inv(cam_colors\proj_colors);
%    figure(11); 
%       plot([cam_colors(:,1:1e3:end); proj_colors(:,1:1e3:end)]'); grid on;
%    figure(12); drawnow;
%       plot([inv_color_calib{camIndex}*cam_colors(:,1:1e3:end); ...
...
load([basedirname, '\proj\calib_data.mat']);

if ~exist('C:\Users\user\Documents\MATLAB\Geometric\proj\v1\ext_calib_data.mat','file')
   load('C:\Users\user\Documents\MATLAB\Geometric\proj\v1\calib_data','n_ima')
   for i = 1:n_ima
      frame = ['C:\Users\user\Documents\MATLAB\Geometric\proj\v1\IMG_',num2str(i,'%d')];
      [Tc,Rc,H] = ...
         computeExtrinsic(fc,cc,kc,alpha_c,frame,'tif',...
                          proj_cal_nX,proj_cal_nY,proj_cal_dX,proj_cal_dY);
      eval(['Rc_',int2str(i),' = Rc']);
      eval(['Tc_',int2str(i),' = Tc']);
      if i == 1
         save('C:\Users\user\Documents\MATLAB\Geometric\proj\v1\ext_calib_data.mat',['Rc_',int2str(i)]);
      else
         save('C:\Users\user\Documents\MATLAB\Geometric\proj\v1\ext_calib_data.mat',['Rc_',int2str(i)],'-append');
      end
      save('C:\Users\user\Documents\MATLAB\Geometric\proj\v1\ext_calib_data.mat',['Tc_',int2str(i)],'-append');
   end
else
   load('C:\Users\user\Documents\MATLAB\Geometric\proj\v1\ext_calib_data');
end
for i = 1:n_ima
...
I_proj1 = imread('.\proj\v1\IMG_1.tif');
...
for i = useProjImages
   ...
   V = eval(['pixel2ray(xproj_',int2str(i),',fc{1},cc{1},kc{1},alpha_c{1});']);
   C = zeros(3,size(V,2));
...
nx           = 1920;        % number of projector columns
ny           = 1080;         % number of projector rows
...
for i = 1
   hold on;
      Y     = Rc_1{i}*X_1{i} + Tc_1{i}*ones(1,size(X_1{i},2));
      Yx    = zeros(n_sq_x_1{i}+1,n_sq_y_1{i}+1);
      Yy    = zeros(n_sq_x_1{i}+1,n_sq_y_1{i}+1);
      Yz    = zeros(n_sq_x_1{i}+1,n_sq_y_1{i}+1);
...
for i = 1:length(fc)
   ...
   r = 1:ny{i};
   [C,R] = meshgrid(c,r);
   Nc{i} = Rc_1{1}*Rc_1{i}'*pixel2ray([C(:) R(:)]'-1,fc{i},cc{i},kc{i},alpha_c{i});
   Oc{i} = Rc_1{1}*Rc_1{i}'*(-Tc_1{i}) + Tc_1{1};
end

% Save the projector\camera calibration results as calib_proj.mat.
save_command = ...
   ['save .\calib_results\calib_proj ',...
    'fc cc kc alpha_c Rc_1 Tc_1 ',...
    'x_1 X_1 nx ny n_sq_x_1 n_sq_y_1 ',...
    'dX dY ',...
    'inv_color_calib ',...
