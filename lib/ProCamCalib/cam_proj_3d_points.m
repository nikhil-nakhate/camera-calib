...
for i= ind_active
    ...
    if (size(imread([calib_name num2str(first_num+i-1) '.' format_image]), 3) == 3)
        I = double(rgb2gray(imread( [calib_name num2str(first_num+i-1) '.' format_image])));
        display('RGB')
    else
        I = double(imread( [calib_name num2str(first_num+i-1) '.' format_image]));
        display('Gray')
    end
...
fprintf(1,'\n');
%This is only projected image only. Only projected squares, not captured
%image
